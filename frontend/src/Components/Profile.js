...
function MyVerticallyCenteredModalUser(props) {
    ...
    const updateUser =  async  () => {
        try {
            ...
            await axios.post("/api/user/info", body, {headers: {
                    'Authorization': `Bearer ${token}`
                }}).then((response) => {
                setUser(response.data);
                props.onHide();
...
const Profile = () => {
    ...
    return(
        <Container  className="mb-3 mt-3">
            <Row  className="mb-3 mt-3" style={{backgroundColor: 'white', borderRadius: '20',padding:'20'}}>
                ...
                <Button variant="success" size="sm" onClick={() => setModalShow(true)}>Редактировать</Button>
                <MyVerticallyCenteredModalUser user={user}
                                           show={modalShow}
                                           onHide={() => {setModalShow(false);getUser();}}/>
            </Row>
            <Row  className="mb-3 mt-3" style={{backgroundColor: 'white', borderRadius: '20',padding:'20',margin:'20'}}>
